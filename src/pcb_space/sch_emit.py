"""Emit a KiCad 10 .kicad_sch from a Zener/KiCad netlist.

Zener stays the netlist language (`pcb build` / `pcb layout`). pcb-space owns
the schematic *drawing*: box ICs, two-pin R/C/L, local net labels on stubs,
and per-part GND/VCC hats. No `pcb apply schematic`.
"""
...
def emit_schematic(net_text: str, *, title: str = "") -> str:
    ...
    uid = new_uuid()
    title = title or "pcb-space"
    return (
        f'(kicad_sch\n'
        f'\t(version 20260306)\n'
        f'\t(generator "pcb-space")\n'
